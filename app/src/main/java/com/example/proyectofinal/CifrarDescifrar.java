...
public class CifrarDescifrar extends AppCompatActivity {
    ...
    @Override
    protected void onCreate(Bundle savedInstanceState) { super.onCreate(savedInstanceState); setContentView(R.layout.activity_cifrar_descifrar);
        txt_original = (EditText) findViewById(R.id.txtInput);
...
//Para quitar los espacios
        ...
        int temp = bloques;
        int grupos = sinEspacio.length() / bloques; int residuo = sinEspacio.length() % bloques; boolean res = false; System.out.println("grupo: " + grupos); System.out.println("residuo: " + residuo); if (residuo > 0) {
            grupos = grupos + 1;
            res = true; }
        while (contador < grupos) {
            temporal = sinEspacio.substring(pivote, bloques); System.out.println("cadena1: " + temporal); ltBloques.add(temporal);
            contador++;
            pivote = pivote + temp;
            bloques = bloques + temp;
            if (contador == grupos - 1 && res) {
                temporal = sinEspacio.substring(pivote,
                        sinEspacio.length());
    ...
    // Permutacion
    public ArrayList<String> permutacion (ArrayList<String>
                                                  cadenaSustituida , String orden){
        ...
        for (int i=0; i<cadenaSustituida.size();i=i+2){
            ...
            while(contador< cadenaSustituida.get(i).length()){
                int pos =
                        Integer.parseInt(String.valueOf(orden.charAt(contador))); nuevaPalabra = nuevaPalabra+
                        String.valueOf(cadenaSustituida.get(i).charAt(pos-1)); contador++;
            }
            ltPermutacion.set(i,nuevaPalabra);
        }
        for(String n : ltPermutacion){ System.out.println(n);
        }
        return ltPermutacion;
    }
    public ArrayList<String> cambiarPosicion(ArrayList<String>
                                                     cadenaSustituida) {
        System.out.println("Cambio de posicion"); ArrayList<String> ltPosicion = cadenaSustituida;
    ...
    // la lista
    public ArrayList<String> permutacionDeco(ArrayList<String>
                                                     cadenaSustituida, String orden) {
        ArrayList<String> ltPermutacion = cadenaSustituida;
        String nuevaPalabra = "";
        String l1 = "";
        ...
        for (int i = 0; i < cadenaSustituida.size(); i = i + 2) {
            ...
            l7 = "";
            l8 = "";
            while (contador < cadenaSustituida.get(i).length()) { System.out.println("Valor de I: " + i);
                int pos =
                        Integer.parseInt(String.valueOf(orden.charAt(contador))); System.out.println("Posición: " + pos);
                if (pos == 1) {
                    l1 =
                            String.valueOf(cadenaSustituida.get(i).charAt(contador)); }
                if (pos == 2) {
                    l2 =
                            String.valueOf(cadenaSustituida.get(i).charAt(contador)); }
                if (pos == 3) {
            ...
            nuevaPalabra = l1+l2+l3+l4+l5+l6+l7+l8; System.out.println("Valor de comando: " + nuevaPalabra); ltPermutacion.set(i, nuevaPalabra);
        }
        return ltPermutacion;
    }
    ...
    public void Calcular(View view){
        ...
        per=Integer.parseInt(textoSeparado[1]); susti=textoSeparado[2];
        ...
        if(rb1.isChecked() == true){
            ...
            if(Integer.toString(per).length() == (tam_bloq/2)){
                for (int i=1;i<maxVal;i++) {
                    if (Integer.toString(per).charAt(0) != Integer.toString(per).charAt((i))) {
                        ...
                        Toast.makeText(this, "Error: Números repetidos en la permutación", Toast.LENGTH_LONG).show();
                        return;
                    }
                }
            }else{
            ...
            System.out.println(setTexto); resp.setText(setTexto);
            ...
            if(Integer.toString(per).length()==(tam_bloq/2)){
                ...
                Toast.makeText(this,"Error: La permutación tiene tamaño incorrecto",Toast.LENGTH_LONG).show();
            }
            char operacion=susti.charAt(0);
            int nOperacion=Integer.parseInt(String.valueOf(susti.charAt(1)));
            boolean suma=false;
                ...
                textoClaro=permutacionDeco(
                        sustitucion(
                                cambiarPosicion(
                                        permutacionDeco(
                                                sustitucion(
                                                        convertirBloques(txt_original.getText().toString(),tam_bloq/2),
                                                        suma,nOperacion
                                                ) ,Integer.toString(per)
                                        )
                                ),
        ...
        copia.setOnClickListener(new View.OnClickListener() {
                                     ...
                                     public void onClick(View view) {
                                         if(view.getId()==R.id.btnCopiar1) { ClipboardManager clipboard = (ClipboardManager)
                                                 getSystemService(Context.CLIPBOARD_SERVICE);
                                             ClipData clip = ClipData.newPlainText("text", resp.getText().toString());
